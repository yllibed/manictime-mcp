...
// ManicTime may omit the leading sign for positive offsets (e.g. "01-00" instead of "+01-00").
// Detect whether the offset starts with an explicit sign or a digit.
var hasSign = remaining.Length > 0 && (remaining[0] == '+' || remaining[0] == '-');
var offsetLen = hasSign ? 6 : 5;

if (remaining.Length < offsetLen + 1 || remaining[offsetLen] != '_')
{
return false;
}

var offsetSpan = remaining[..offsetLen];
remaining = remaining[(offsetLen + 1)..];

// Validate the '-' separator within HH-mm (position 3 for signed, 2 for unsigned)
var separatorIdx = hasSign ? 3 : 2;
if (offsetSpan[separatorIdx] != '-')
{
return false;
}

// Normalize unsigned offsets to the canonical "+HH-mm" form
offset = hasSign ? offsetSpan.ToString() : "+" + offsetSpan.ToString();
return true;
}
