...
- Expose these Repl-first commands:
  - `timeline list`
  - `activity list`
  - `activity computer-usage`
  - `activity tags`
  - `usage applications`
  - `usage documents`
  - `usage websites`
  ...
  - `screenshot save`
  - `workspace init`
- Derive MCP tools from the Repl command graph. The current flattened MCP names are expected to follow the route path shape:
  - `timeline_list`
  - `activity_list`
  - `activity_computer-usage`
  - `activity_tags`
  - `usage_applications`
  - `usage_documents`
  - `usage_websites`
  ...
  - `screenshot_save`
  - `workspace_init`
- Do not expose the legacy `get_*` / `list_*` naming scheme as the source-of-truth contract.
- Expose these resources:
  - `manictime://resource/config`
  - `manictime://resource/timelines`
...
Notes:
- `screenshot list` is the discovery entry point for screenshot availability and timestamps.
...
```
Parameters:
  screenshotRef: string (opaque reference returned by screenshot list; stable for the MCP session)
...
Region-of-interest extraction using percentage-first coordinates. Designed for model-driven workflows: the model inspects the thumbnail from `screenshot get`, identifies a region of interest, then requests a full-resolution crop. See WS-05 and ADR-0004.
...
```
Parameters:
  screenshotRef: string (opaque reference from screenshot list)
...
- Output path must resolve within a client-declared or session-initialized MCP root (`file:///` URI).
...
- If no native roots are declared by the client, the agent must first call `workspace init` to establish a soft root for the session.
...
Session-scoped helper for establishing soft roots when the MCP client does not support native roots.

Preferred Repl command: `workspace init`
...
Notes:
- `workspace init` is intended primarily for MCP sessions.
- In CLI and REPL mode, callers typically already control the filesystem directly and do not need a session root bootstrap step.
- `screenshot save` must accept these soft roots exactly as it accepts native MCP roots.
...
Replace bare activity/usage counts with narrative output. Internally delegates to `summary narrative` logic to produce structured segments and aggregates. Accepts `maxSegments` parameter (default 200, max 2000) to control segment truncation.
...
- Screenshot workflow: `screenshot list` -> `screenshot get` -> `screenshot crop` -> `workspace init` when no native roots -> `screenshot save`.
...
- Screenshot availability guide:
  - always discover via `screenshot list` first
...
Returns a structured JSON resource payload. Useful for diagnostics and compatibility reporting.
...
Allows models to know data boundaries without querying activities. Returns a structured JSON resource payload.
...
Prompt text:
  "Use summary narrative with the date-derived range and includeSummary=true
   ...
   present, use screenshot get + screenshot crop + workspace init when no
   native roots + screenshot save to
...
Prompt text:
  "Use summary period with the provided date range to retrieve
   ...
   screenshot get + screenshot crop + workspace init when no native roots +
   screenshot save for visual context."
```
...
Prompt text:
  "Use screenshot list to find screenshots near {datetime} (within a
   ...
   refs. If you need a durable image artifact, use workspace init when no
   native roots are present, then call screenshot save."
```
...
- Soft-roots workflow tests (`workspace init` -> `screenshot save`) for clients without native roots.
